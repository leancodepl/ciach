...
| `tool/render_assets.mjs` | Regenerates the PNG icons from `favicon.svg` and the Open Graph card from the site's own stylesheet and web fonts, with Playwright: `PLAYWRIGHT_CHROMIUM=<path> node tool/render_assets.mjs` (needs `playwright` resolvable from `tool/`, and `curl` able to reach Google Fonts). |
...
- `jaspr_builder` 0.23 requires `analyzer` 12 and does not compile against
  ...
  versions that fail to compile. Three lint rules whose fixes produce Dart
  3.13 primary constructors are off, since analyzer 12 only parses those
  behind an experiment flag that `jaspr` cannot pass to build_runner. Drop
  all of this once Jaspr moves to a newer analyzer.
